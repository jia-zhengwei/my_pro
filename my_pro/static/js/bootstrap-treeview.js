...
;(function ($, window, document, undefined) {
	...
	var Tree = function (element, options) {
		...
		return {
			...
			//添加右击事件
			onMouseDown: $.proxy(this.onMouseDown, this),
    ...
    */
	...
	Tree.prototype.subscribeEvents = function () {
		...
		this.$element.on('contextmenu', $.proxy(this.clickHandler, this));
		// this.$element.on('mouseover', $.proxy(this.mouseLeave, this));
	...
	*/
	...
	鼠标划过事件的处理
	*/
	// Tree.prototype.mouseLeave = function (event) {
	...
	// 	});
	// 	function tips(){
	// 		layer.tips(node.text, target, {
	// 			time: 1000,
	// 		});
	// 	};
	// 	setTimeout(tips, 1000);
	// 	// setTimeout("layer.tips('Hi，我是tips', target)", 1000);

	// };

	// 查找要检索的最接近的父列表项的DOM
	//数据属性nodeid，用于查找扁平化结构中的节点。
	...
	Tree.prototype.render = function () {
		...
		if (!this.initialized) {
			...
			// Setup first time only components设置第一次只有组件
	...
	*/
	...
	/**
		切换节点选择状态; 选择是否被选中，如果选择则取消选择。
		@param {Object|Number} 标识符 - 节点标识符的有效节点，节点ID或数组
		@param {optional Object} options
	*/
	...
	*/
	...
	/**
		查找符合给定条件的节点
		@param {String} 模式 - 给定的字符串进行匹配
		@param {optional String} 修饰符 - 有效的RegEx修饰符
		@param {optional String} 属性 - 比较模式的属性
		@return {Array} 节点 - 符合条件的节点
	*/
